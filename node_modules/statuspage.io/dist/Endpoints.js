...
(function (Endpoint) {
    ...
    (function (Incidents) {
        function all() {
            return `/${API_V1_BASE}/${INCIDENTS_JSON}`;
        }
        Incidents.all = all;
        function unresolved() {
            return `/${API_V1_BASE}/${INCIDENTS}/${UNRESOLVED_JSON}`;
        }
    ...
    (function (ScheduledMaintenances) {
        function active() {
            return `/${API_V1_BASE}/${SCHEDULED_MAINTENANCES}/${ACTIVE_JSON}`;
        }
        ScheduledMaintenances.active = active;
        function all() {
            return `/${API_V1_BASE}/${SCHEDULED_MAINTENANCES_JSON}`;
        }
        ScheduledMaintenances.all = all;
        function upcoming() {
            return `/${API_V1_BASE}/${SCHEDULED_MAINTENANCES}/${UPCOMING_JSON}`;
        }
    ...
    function components() {
        return `/${API_V1_BASE}/${COMPONENTS_JSON}`;
    }
    Endpoint.components = components;
    function summary() {
        return `/${API_V1_BASE}/${SUMMARY_JSON}`;
    }
    Endpoint.summary = summary;
    function status() {
        return `/${API_V1_BASE}/${STATUS_JSON}`;
    }
    Endpoint.status = status;
    function subscribers() {
        return `/${API_V1_BASE}/${SUBSCRIBERS_JSON}`;
    }
