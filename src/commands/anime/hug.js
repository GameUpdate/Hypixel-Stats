...
module.exports = {
    config: {
        name: "hug",
        desc: "Hug someone or everyone",
        usage: [`hug <user>`, `hug everyone`]
    },
    run: async (server, message, args) => {
        ...
        if (user === message.author) {
            noperms.setDescription(`You hugged yourself!`)
        } else if (user === 'all') {
            noperms.setDescription(`${message.member} hugged everyone!`)
        } else {
            noperms.setDescription(`${message.member} hugged ${user}!`)
        }

        let baka = await neko.sfw.hug()
