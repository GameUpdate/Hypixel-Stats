...
module.exports = {
    config: {
        name: "join",
        admin: true,
        desc: `Set up what happens when a new member joins and test what you have`,
        usage: [`join`, `join image`, `join channel <channel>`, `join message <message>`, `join message var`, `join test`]
    },
    run: async (server, message, args) => {
        const color = server.color

        if (!args[0]) {
            server.welcome.enabled ? server.welcome.enabled = false : server.welcome.enabled = true
            server.save().then(() => { return message.channel.send(`Join message has been turned ${server.welcome.enabled ? `**ON**` : `**OFF**`}`) })

        } else if (args[0].toLowerCase() === 'image') {
            server.welcome.image ? server.welcome.image = false : server.welcome.image = true
            server.save().then(() => { return message.channel.send(`Join message image has been turned ${server.welcome.image ? `**ON**` : `**OFF**`}`) })
            ...
            server.save().then(() => { return message.channel.send(`Join message channel has been set to ${channel.toString()}`) })
            ...
            if (args[1].toLowerCase() != 'var') {
                args.shift()
                server.welcome.message = args.join(" ")
                server.save().then(() => { return message.channel.send(`Join message has been set to \`\`\`${args.join(" ")}\`\`\``) })
            } else {
